...
'''
@Author: Youshumin
@Date: 2019-11-12 17:32:03
@LastEditors: Youshumin
@LastEditTime: 2019-11-12 17:32:05
@Description: 
'''
import os
import random
...
fonts_dir = os.path.abspath(os.path.join(options.APP_PATH, "data/fonts"))
...
class Captcha(object):
    ...
    def __init__(self, width, height, fonts=None, color=None):
        ...
        self._fonts = fonts if fonts else [
            os.path.join(fonts_dir, font) for font in [
                "allgemeine.woff.ttf",
            ]
        ]
        self._color = color if color else random_color(
            0, 200, random.randint(200, 255))
        self._width, self._height = width, height

    @classmethod
    def instance(cls, width=200, height=75):
        prop_name = '_instance_{width}_{height}'.format(width=width,
                                                        height=height)
    ...
    def generate(self, captcha_text='', fmt='PNG'):
        ...
        self.text(captcha_text,
                  ...
                  color=random_color(0, 200, random.randint(200, 255)))
        self.curve()
        self.noise()
        self.smooth()
        image_bytes = BytesIO()
...
ALL_CHARS = '23456789abcdefghkmnpqrstuvwxyzABCDEFGHKMNPQRSTUVWXYZ'
